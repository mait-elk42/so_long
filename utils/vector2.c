...
/*   Updated: 2024/01/15 14:41:09 by mait-elk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	_move_to(t_mlx	*mlx_info, int x, int y)
{
	...
	if (mlx_info->maps[y][x] == '0')
	{
		...
		mlx_put_image_to_window(mlx_info->mlx_ptr, mlx_info->win_ptr, mlx_info->Floor->sprite, 32, 32);
		//FIX THE COLOR !!
		mlx_string_put(mlx_info->mlx_ptr, mlx_info->win_ptr, 32, 32, _nsx_gettrgb(255, (mlx_info->player.steps_count % 255), 255 - (mlx_info->player.steps_count % 255), 0), ft_itoa(mlx_info->player.steps_count));
	}
}
